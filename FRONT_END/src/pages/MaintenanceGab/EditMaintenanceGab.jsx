...
const EditMaintenanceGab = () => {
  ...
  const [isFetchContratDone,SetIsFetchContratDone]=useState(false)
  ...
  useEffect(() => {
    if(isFetchContratDone){
      RefreshRedevance()
      SetIsFetchContratDone(false)
     }
  }, [contrat.redevance_gab]);
  ...
  const RefreshRedevance= ()=>{

    let isQ1Exist = getRedevance('Q1',contrat.redevance_gab)
    let isQ2Exist = getRedevance('Q2',contrat.redevance_gab)
    let isQ3Exist = getRedevance('Q3',contrat.redevance_gab)
    let isQ4Exist = getRedevance('Q4',contrat.redevance_gab)

    if (typeof isQ1Exist === "object"){
        // console.log('Q1 exist ');
        setShowQ1(true)
        setQ1(defaultQ1())
    }
    if(typeof isQ1Exist === "undefined"){
      // console.log('Q1 not exist ');
      setQ1( {
        type: "Q1",
        num_facture: "",
        date_facture: null,
    ...
    if (typeof isQ2Exist === "object"){
        // console.log('Q2 exist ');
        setShowQ2(true)
        setQ2(defaultQ2())
    }
    if(typeof isQ2Exist === "undefined"){
      // console.log('Q2 not exist ');
      setQ2({
        type: "Q2",
        num_facture: "",
        date_facture: null,
    ...
    if (typeof isQ3Exist === "object"){
        // console.log('Q3 exist ');
        setShowQ3(true)
        setQ3(defaultQ3())
    }
    if(typeof isQ3Exist === "undefined"){
      // console.log('Q3 not exist ');
      setQ3( {
        type: "Q3",
        num_facture: "",
        date_facture: null,
    ...
    if(typeof isQ4Exist === "object"){
        // console.log('Q4 exist ');
        setShowQ4(true)
        setQ4(defaultQ4())
    }
    if(typeof isQ4Exist === "undefined"){
      // console.log('Q4 not exist ');
      setQ4( {
        type: "Q4",
        num_facture: "",
        date_facture: null,
  ...
  const getContrat = async () =>{
    ...
    try {
     ...
     const redevance_gab = contrat_data?.redevance_gab.map((redevance_)=>({...redevance_,
      date_facture:  redevance_.date_facture ? date_format(redevance_.date_facture) : null,
      num_facture: redevance_.num_facture ? redevance_.num_facture : ''
    }))

    let contrat_ = {
      client: contrat_data?.client,
      date_debut: contrat_data?.date_debut ? date_format(contrat_data?.date_debut) : null,
      ...
      date_proposition: contrat_data?.date_proposition ? date_format(contrat_data?.date_proposition) : null,
      observation : contrat_data?.observation ? contrat_data?.observation : '',
      redevance_gab: redevance_gab
    }
    SetIsFetchContratDone(true)
    setContrat(contrat_)
  }
  ...
  const getEquipement = async () => {
    ...
    try {
      const { data } = await axios.get(
        `${API_URL}/api/equipement/edit_to_contrat_maintenance_gab/${equipement_id}`
      );
      if (data) {
    ...
    if (
      ...
      if (
        ...
        try {
          ...
          const data_ = await axios.put(
            `${API_URL}/api/contrat_maintenance_gab/${id}`,
            updatedContrat
            );
  ...
  return (
    ...
    >
      ...
      <div style={headerLabelContainer}>
        ...
        <HeaderLabel title="MODIFICATION CONTRAT DE MAINTENANCE GAB" />
      </div>
...
export default EditMaintenanceGab;